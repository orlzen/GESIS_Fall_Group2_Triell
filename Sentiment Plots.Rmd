...
ARD %>%
 ggplot(aes(x= time, y= sentiment,  group=candidate_comb ,color=candidate_comb)) +
    geom_line() + theme(legend.position="bottom") + labs(title = "Sentiments in Twitter Debate During ARD Triell", color='Candidates') + scale_color_manual(values = c("green", "black", "red")) + guides(x=guide_axis(n.dodge = 4))
```
