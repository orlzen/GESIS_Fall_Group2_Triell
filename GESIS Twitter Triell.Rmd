...
For this we get the sentiments of all the tweets, using the `Syuzhet` package, which can also be used with German (setting `lang <- "german"` and using `method = "nrc"` instead of `affin`, as `affin` is not availiable in German).

In the following code chunk we execute the sentiment analysis for the the two datasets ("large" and "all").
...
We can run a keyness analysis to find differences in th e 
