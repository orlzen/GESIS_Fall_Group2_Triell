...
> *How does the Twitter discussion About the three candidates in the context of the ARD triell Looks like?*
...
The sentiments during the Triell can then be plotted over time.
...
df_tablRet <- triell_all %>% 
  group_by(candidate_comb, retweet_count) %>% 
  summarise(sum=(retweet_count))

df_tablRet
rename(df_tablRet, sum(retweet_count) = retweet_count_candidates)

ggplot(data = df_tablRet) +
  geom_bar(mapping = aes(x = "sum(retweet_count)"),
                 binwidth = 0.5)
