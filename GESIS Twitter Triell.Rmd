...
This gave us two dataframes; one with the combined searched tweets of the candidates; called `triell_all` of `r count(triell_all)` entries and one large dataframe called `triell_large` of `r count(triell_large)` entries.
...
In the dataset `triell_all` we were 
...
We can also convert the `triell` dataframes into a corpus, but would need a German lemmatization word list.
...
We will try to find differences in the tweets, depending on who (which candidate, or which comibination of candidates is mentioned in the tweets).
